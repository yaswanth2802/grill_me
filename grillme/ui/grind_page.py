...
def render_grind_page():
    ...
    if not st.session_state.grind_started:
        ...
        with st.spinner("⏳ Running setup: analyzing resume and JD..."):
            ...
            # Ensure the first interviewer question is added to conversation history
    ...
    if st.session_state.grind_started:
        ...
        # Conversation history headers / progress
        col1, col2 = st.columns([3, 1])
        with col1:
                ...
                if st.button("📤 Submit Answer", key="submit_btn"):
                    if not user_answer.strip():
                        ...
                        with st.spinner("⏳ Evaluating answer..."):
                            ...
                            # Safe string conversion for conductor response
                            if result_state.get("agent_response"):
                                raw_resp = result_state["agent_response"].response_text
                                conductor_response = "".join([item.get("text", "") for item in raw_resp]) if isinstance(raw_resp, list) else str(raw_resp)
